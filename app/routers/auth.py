...
router = APIRouter(prefix="/auth", tags=["auth"]) # Define el prefijo de las rutas y las etiquetas

@router.post("/request-otp", response_model=MessageOut) # Define la ruta y el modelo de respuesta
def request_otp(payload: RequestOTPIn, db: Session = Depends(get_db)): # payload es el cuerpo de la petición y db es la sesión de la base de datos
    ...
    existing_user_for_house = db.query(User).filter(User.household_id == household.id).first() # Busca si ya existe un usuario para esa vivienda
    ...
    # 4) DEV: imprimir OTP en consola (en producción, enviar por email/telegram) 
    ...
    def verify_otp(payload: VerifyOTPIn, db: Session = Depends(get_db)): 
        ...
        if not verify_secret(payload, otp_row.otp_hash): 
