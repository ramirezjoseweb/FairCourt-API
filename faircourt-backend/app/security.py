...
def create_checkin_token(reservation_id: int, expires_at: datetime) -> str: 
    """
    Crea un token JWT para check-in QR de una reserva concreta
    """
    payload = {
        "purpose": "checkin",
    ...
    return jwt.encode(payload, settings.SECRET_KEY, algorithm=settings.JWT_ALG) # jwt.encode es una función que codifica el payload en un token

def decode_checkin_token(token: str) -> dict: # dict es un tipo de dato que almacena datos en formato clave-valor
    """
    Decodifica un token JWT de check-in y devuelve el payload
    """
    return jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.JWT_ALG]) # jwt.decode es una función que decodifica el payload en un token